...
parser.add_argument('--session_name', type=str, default='session_v4', help='Session identifier.')
parser.add_argument('--lr', type=float, default=1e-5, help='Initial learning rate.')
parser.add_argument('--epochs', type=int, default=100, help='Number of epochs.')
parser.add_argument('--layers_to_unfreeze', type=int, default=50, help='Layers to unfreeze.')
...
data_path = '/home/yat20006/BigDataProjectAttempt2/Vehicles/predicting_vehicles/archive'
...
def calculate_class_weights(train_sources):
...
def data_generator(data_sources, label_count):
    while True:
        img_batches = []
        label_batches = []
        ...
        if not img_batches:
            continue
        yield (
            np.concatenate(img_batches),
            np.concatenate(label_batches)
        )

train_sources, val_sources, train_total, val_total = load_data()
class_weights = calculate_class_weights(train_sources)

train_gen = data_generator(train_sources, label_count)
val_gen = data_generator(val_sources, label_count)
...
model_name = 'ResNet50'
print(f"Starting {model_name} training in session '{session_name}'")
...
def adjust_lr(epoch, current_lr):
    decay_rate = 0.1
    decay_step = 30
...
callbacks = [
    ...
    LearningRateScheduler(adjust_lr, verbose=1)
]
...
training_data = log_data.to_dict(orient='list')
